...
async def handle_remember(router: MemoryBankRouter, arguments: dict) -> dict:
    ...
    from src.services.tools.deduplication import extract_file_hash
...
async def handle_forget(router: MemoryBankRouter, arguments: dict) -> dict:
    ...
    file_repo = FileRepositorySQLite(conn_manager)
    chunk_repo = FileChunkRepositorySQLite(conn_manager)
    relation_repo = FileRelationRepositorySQLite(conn_manager)
...
async def handle_sleep(router: MemoryBankRouter, arguments: dict) -> dict:
    ...
    use_case = SleepMemoryUseCase(
        mnemosyne_client=instance,
        logger=logger,
    )
    result = use_case.execute(params)
    return _raise_on_ko(result, "sleepMemory")
