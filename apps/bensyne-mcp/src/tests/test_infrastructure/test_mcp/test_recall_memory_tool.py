...
class TestRecallMemoryToolRegistration:
    ...
    def test_tool_registry_exactly_16_tools(self) -> None:
        """Registry holds exactly 16 tools: the 12 memory/file tools plus the
        operator-only forgetFile, getPersonaStatus, getPersonaEntryNode and
        searchMemoryBank. Adding a param must NOT add a tool."""
        from src.app import register_tools
...
class TestFetchFileNeighborParams:
    ...
    def test_registry_exactly_16_tools_after_neighbor_params(self) -> None:
        """Adding neighbor params must NOT add a new tool — registry stays at exactly
        16 (12 memory/file tools + operator-only forgetFile + getPersonaStatus +
        getPersonaEntryNode + searchMemoryBank)."""
        from src.app import register_tools
