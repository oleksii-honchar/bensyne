...
see_also: [
  ...
  "adrs/0029-disable-enrichment-by-default.adr.md",
...
⚠️ **Current status: DISABLED by default.** The original rationale — that enriched metadata provides additional retrieval signals — is incorrect. The Mnemosyne recall system does not index the `metadata_json` field where enriched metadata is stored. See [[0012-enrichment-metadata-not-indexed]] for details and [[0029-disable-enrichment-by-default]] for the decision to disable.
...
- **Configuration:** `enrichment.enabled`, `enrichment.llmUrl`, `enrichment.llmModel`, `enrichment.apiKey` — all three must be set (AND guard); `enabled` defaults to `false` per ADR-0029
...
The `metadata_json` field is a storage field, not a search field. Enrichment adds latency (LLM call per document) without improving retrieval. See [[0012-enrichment-metadata-not-indexed]] and [[0029-disable-enrichment-by-default]].
